...
ggplot(meta_data_all_years, aes(x = as.character(year), y = as.numeric(spots)))+
...
ggsave(paste0(out, "sra_RNAseq_300_2009-2024.pdf"), width = 5, height = 3)

median(as.numeric(meta_data_all_years$spots), na.rm = T)
...
ggplot(meta_data_all_years_spl, aes(x = as.character(year), y = as.numeric(spots)))+
  ggrastr::rasterise( gghalves::geom_half_point(alpha = 0.2, color = "grey50"), dpi= 300)+
  gghalves::geom_half_boxplot()+
  scale_y_log10()+
  geom_hline(yintercept = 30000000, color = "darkorange")+
  ylab("Read count \n per sample [log10]")+
