...
The influence of read length on read alignment is determined by plotting the fraction of unique, multiple and unmapped reads 
for each read length (2 replicates each for knockdown and control).The alignment statistics are obtained from STAR. 
...
StarReport_StackedBarChart = annotate_figure(StarReport_StackedBarChart, 
                                             top = text_grob("Influence on read alignment", size = 12, 
                                             just = "centre", face = "bold"))
```

# 2.1 - Influence on LSV detection (Fig 2A)

The influence of read length on LSV detection is determined by counting the LSV hits (rows) 
in the tsv files received from MAJIQ for each read length. 
...
The LSV complexity describes how many junctions an LSV has. To measure the influence on this, the LSVs
with 2, 3, 4 or more than 4 junctions are counted for each read length. 
...
Influence of read length on the significance of LSVs is determined by plotting the fraction of significantly 
regulated LSVs detected for each read length. LSVs are considered significant if dPSI is at least 0.05 and 
probability changing is at least 0.9. 
...
Influence on the size of detected splicing changes is determined by plotting the cumulative fraction of dPSIs 
and probability changing values for each read length. 
...
Since there are alternative splicing events with different numbers of junctions, they are treated 
differently in terms of significance. 

Alternative splicing events with two junctions (A3SS, A5SS, AFE, ALE, IR) are considered significantly 
regulated if at least one junction has a dPSI of at least 0.05 and a probability changing value of at least 0.9. 
Events with 4 junctions (CE and MEE) are considered significantly regulated if either both junctions starting from C1 
or both junctions starting from C2 fulfill the regulation criteria. 
