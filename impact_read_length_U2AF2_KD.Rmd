...
```{r}
# load star log files 
StarDir = "C:/Users/ladwi/Desktop/HiWi/project_impact_read_length/U2AF2/STAR"
StarLogFiles=list.files(path=StarDir)
setwd(StarDir)
StarLogFiles = lapply(StarLogFiles, read.delim)

# function to calculate fraction of splices (junction-spanning reads) among uniquely mapping reads
calculate_fraction_splices <- function(df) {
  fracs <- as.numeric(df[10, 2])/as.numeric(df[7, 2])
  fraction_splices <- c(fracs)
}

# calculate the fraction (see above)
fraction_splices = lapply(StarLogFiles, calculate_fraction_splices)

# get means of the fractions for each read length
idxs <- list(c(1, 5, 9, 13), c(2, 6, 10, 14), c(3, 7, 11, 15), c(4, 8, 12, 16))
means <- sapply(idxs, function(read_length) mean(unlist(fraction_splices[read_length])))
means
...
dpsiDir = "C:/Users/ladwi/Desktop/HiWi/project_impact_read_length/U2AF2/MAJIQ/read_length"
...
# upset plot
pdf(file="C:/Users/ladwi/Desktop/HiWi/project_impact_read_length/plots/upset_read_length.pdf", width=3.6, height=3) 
...
ggsave(filename = "C:/Users/ladwi/Desktop/HiWi/project_impact_read_length/plots/sharedRegulation_100_50_readLength.pdf", plot = sharedRegulation_scatterPlot2, width = 5.65, height = 5.65, units = "cm")
```
...
modDir = "C:/Users/ladwi/Desktop/HiWi/project_impact_read_length/U2AF2/Modulize/read_length"
...
filter2JEvents <- function(eventTable) {
  ...
  eventTable = eventTable %>% group_by(event_id) %>% slice(1)
  }
...
names(significantEvents_2J) = c("A3SS_36", "A3SS_50", "A3SS_75", "A3SS_100", 
                                "A5SS_36", "A5SS_50", "A5SS_75", "A5SS_100", 
                                "AFE_36", "AFE_50", "AFE_75", "AFE_100", 
                                "ALE_36", "ALE_50", "ALE_75", "ALE_100", 
                                "IR_36", "IR_50", "IR_75", "IR_100")
names(significantEvents_4J) = c("CE_36", "CE_50", "CE_75", "CE_100",
                                "MXE_36", "MXE_50", "MXE_75", "MXE_100")
...
numberOfEventTypes_100_df = data.frame(event = c("A3SS", "A5SS", "AFE", "ALE", "IR", "CE", "MXE"), 
                                       n = c(nrow(significantEvents_2J$A3SS_100), 
                                             ...
                                             nrow(significantEvents_4J$MXE_100)))

numberOfEventTypes_75_df = data.frame(event = c("A3SS", "A5SS", "AFE", "ALE", "IR", "CE", "MXE"), 
                                       n = c(nrow(significantEvents_2J$A3SS_75), 
                                             ...
                                             nrow(significantEvents_4J$MXE_75)))

numberOfEventTypes_50_df = data.frame(event = c("A3SS", "A5SS", "AFE", "ALE", "IR", "CE", "MXE"), 
                                       n = c(nrow(significantEvents_2J$A3SS_50), 
                                             ...
                                             nrow(significantEvents_4J$MXE_50)))

numberOfEventTypes_36_df = data.frame(event = c("A3SS", "A5SS", "AFE", "ALE", "IR", "CE", "MXE"), 
...
numberOfEventTypes_cols = 
  ...
  xlab("event type") +
  ylab("# events") +
  scale_fill_manual(values = c("036" = "#FAD8B4", "050" = "#F7B56D", "075" = "#DE872A", "100" = "#C85813"), 
                    name = "read length [nt]", 
                    labels = c("36", "50", "75", "100"))
...
numberOfEventTypes_StackedBarChart = 
  ...
  scale_y_log10() +
  xlab("read length [nt]") +
  ylab("fraction significant events (log10)") +
  #scale_x_discrete(labels = c("36", "50", "75", "100")) +
...
# save plots
#ggsave(filename = "C:/Users/ladwi/Desktop/HiWi/project_impact_read_length/plots/nEventTypes_RL.pdf", plot = numberOfEventTypes_cols, width = 11, height = 7, units = "cm")
ggsave(filename = "C:/Users/ladwi/Desktop/HiWi/project_impact_read_length/plots/distrEventTypes_RL.pdf", plot = numberOfEventTypes_StackedBarChart, width = 4.8, height = 7, units = "cm")
...
# significant events for each read length
...
# In group 454: `event_id = "ENSG00000067225.21_2_AI_3"`.
# was ist AI
```
...
# !data frame so ausbauen, dass es noch spalte für den event typen gibt und dann danach gruppieren
# !nicht rep 4, sondern mal die anzahl
significantEvents_df = data.frame(comparison=c(rep("036", nrow(filteredEvents_2J[[1]])),
                                               rep("050", nrow(filteredEvents_2J[[2]])), 
                                               rep("075", nrow(filteredEvents_2J[[3]])), 
                                               rep("100", nrow(filteredEvents_2J[[4]]))),
                                  significant=c(filteredEvents_2J[[1]]$significant,
                                                filteredEvents_2J[[2]]$significant, 
                                                filteredEvents_2J[[3]]$significant, 
                                                filteredEvents_2J[[4]]$significant)
                                  )

# differ between the significance of events of different read lengths to plot them in different colors 
for (i in 1:nrow(significantEvents_df)){
  if (significantEvents_df[i, "comparison"] == "036" && significantEvents_df[i, "significant"] == TRUE){
    significantEvents_df[i, "significant"] = "TRUE read length 036"
  }
  if (significantEvents_df[i, "comparison"] == "050" && significantEvents_df[i, "significant"] == TRUE){
    significantEvents_df[i, "significant"] = "TRUE read length 050"
  }
  if (significantEvents_df[i, "comparison"] == "075" && significantEvents_df[i, "significant"] == TRUE){
    significantEvents_df[i, "significant"] = "TRUE read length 075"
  }
  if (significantEvents_df[i, "comparison"] == "100" && significantEvents_df[i, "significant"] == TRUE){
    significantEvents_df[i, "significant"] = "TRUE read length 100"
  }
}

# stacked bar chart showing fraction of significant events among all detected events for each read length   
