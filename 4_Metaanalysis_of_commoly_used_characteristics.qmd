...
a <- all_encode_RNAseq %>% subset(organism == "Homo sapiens") 
median(a$mean_read_count, na.rm = T)

b<- all_encode_RNAseq %>% subset(organism == "Mus musculus") 
median(b$mean_read_count, na.rm = T)
...
all_encode_RNAseq <- mutate(all_encode_RNAseq, year_groups = 
...
ggplot(all_encode_RNAseq_int_org1, aes( x = as.numeric(mean_read_count),  color =year_groups)) +
...
ggsave( filename = paste0(out, "Encode_read_count_hs_mm_ecdf.pdf"), width = 8, height = 4)


ggplot(all_encode_RNAseq_int_org2, aes( x = as.numeric(mean_read_count),  color =year_groups)) +
  geom_vline(xintercept = 30000000, size = 1, color = "grey40")+
  geom_vline(xintercept = 10000000, size = 1, color = "grey40")+
  geom_vline(xintercept = 50000000, size = 1, color = "grey40")+
 stat_ecdf(size = 1)+
  scale_color_manual( values = color)+
  scale_x_log10()+
  facet_wrap(~organism, ncol = 4, scales = "free_x")+
...
#ggsave( filename = paste0(out, "Encode_read_count_dm_ss_ecdf.pdf"), width = 8, height = 3)
