...
# Plot for paper
all_encode_RNAseq_int_org <- subset(all_encode_RNAseq, organism %in% c("Homo sapiens", "Mus musculus", "Caenorhabditis elegans", "Drosophila melanogaster"))
all_encode_RNAseq_int_org$organism <- factor(all_encode_RNAseq_int_org$organism, levels = c("Homo sapiens", "Mus musculus", "Drosophila melanogaster", "Caenorhabditis elegans"))

p0 = ggplot(all_encode_RNAseq_int_org, aes(y = as.numeric(mean_read_count), x = as.character(year), group =year)) +
  ggrastr::rasterise( gghalves::geom_half_point(alpha = 0.2, color = "grey50", size = 0.8), dpi = 300)+
  gghalves::geom_half_boxplot()+
  geom_hline(yintercept = 30000000, color = "darkred", size = 1)+
  geom_hline(yintercept = 1000000, color = "darkorange", size = 1)+
  scale_y_log10()+
...
ggsave(p0, filename = paste0(out, "Encode_read_count.pdf"), width = 10, height = 3)
