...
# adjust row and column names for heatmap
...
pdf(paste0(out, "heatmap_quantifiable_introns.pdf"), width = 8, height = 13)
...
# table
kable(as.data.frame(overlaps_quant_m)) %>%
  kableExtra::scroll_box(width = "100%")

```
## Plot read depth and read length separately 

(for supplements)

```{r}
gg_df <- overlaps_quant
gg_df$n_reads <- factor(gg_df$n_reads, levels = unique(gg_df$n_reads))

ggplot(gg_df, aes(x = as.numeric(read_length), color = n_reads, y = n_quantifiable)) +
  ...
  scale_y_continuous(breaks = seq(0, 10e5, by = 2e5), labels = scales::label_number(scale = 1))+
  theme(legend.position = "top")+
  scale_x_continuous(breaks = c(36, 50, 75, 100, 125, 150))

ggsave(paste0(out, "read_length_quantifiable_intr.pdf"), width = 6, height = 7 )

gg_df <- overlaps_quant
gg_df$read_length <- factor(gg_df$read_length, levels = c("36", "50", "75", "100", "125", "150"))

ggplot(gg_df, aes(x = as.numeric(n_reads), color = read_length, y = n_quantifiable)) +
  geom_point(stat = "identity", position = "dodge") +
  labs(x = "Read Depth", y = "Number of quantifiable introns (10 intron-spanning reads)", fill = "Number of reads") +
...
ggsave(paste0(out, "read_depth_quantifiable_intr.pdf"), width = 6, height = 7 )

```

## Plot read depth vs read length

```{r}

gg_df$n_quantifiable_round <- floor(gg_df$n_quantifiable / 50000) * 50000
gg_df$read_length <- as.numeric(as.character(gg_df$read_length))

ggplot(gg_df, aes(x = 
as.numeric(read_length), y = as.numeric(n_reads), color = as.character(n_quantifiable_round)))+
  geom_point()+
  geom_line()


gg_df2 <- gg_df %>% subset(n_quantifiable_round %in% c(600000, 700000, 800000))


ggplot(gg_df2, aes(x = 
read_length, y = as.numeric(n_reads) , color = as.character(n_quantifiable_round)))+
  #scale_y_log10()+
  geom_point()+
  geom_line()+
  scale_x_continuous(breaks = c(36, 50, 75, 100, 125, 150))+
  scale_y_continuous(breaks = c(0, 10000000, 30000000, 50000000, 75000000, 100000000, 150000000, 200000000))+
  scale_color_manual(values = c("deeppink", "deeppink3", "darkorchid4"), labels = c("600K", "700K", "800K"))+
  xlab("read length")+
  ylab("read depth ")+
  theme(legend.position = "none") +
  expand_limits(y = 0)

ggsave(paste0(out, "read_depth_vs_length_quantifiable_intr.pdf"), width = 6, height = 6)
...
## Sanity check ... on expression of genes with introns
